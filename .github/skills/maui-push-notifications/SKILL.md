...
```csharp
// Avoid: omit the trailing slash
_http = new HttpClient { BaseAddress = new Uri("https://example.com") };

// Prefer: include the trailing slash
_http = new HttpClient { BaseAddress = new Uri("https://example.com/") };
```

### Android: always re-register on token refresh
...
```csharp
// Avoid: register only once at startup
// (token may change later without re-registration)

// Prefer: re-register in OnNewToken
