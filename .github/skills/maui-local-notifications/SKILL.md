...
```csharp
// Correct for API 31+: include Immutable
var pendingIntentFlags = (Build.VERSION.SdkInt >= BuildVersionCodes.S)
    ? PendingIntentFlags.CancelCurrent | PendingIntentFlags.Immutable
    : PendingIntentFlags.CancelCurrent;

// Avoid: omit Immutable on Android 12+
...
// Avoid: always use Alert on iOS 14+
completionHandler(UNNotificationPresentationOptions.Alert);
```

### Windows

Windows App SDK supports toast notifications, but scheduled notifications are not currently supported. Immediate notifications work.

## DI registration — platform-specific only

```csharp
// Use #if guards because there is no single cross-platform implementation
#if ANDROID
