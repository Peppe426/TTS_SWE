...
description: >
  ...
  DO NOT USE FOR: full geolocation workflows,
  media capture flows, or notification delivery patterns (use maui-local-notifications).
---
...
```csharp
// Avoid: request immediately without checking current status
...
```csharp
// Avoid: block in a constructor
public MyViewModel()
{
    var status = Permissions.RequestAsync<Permissions.Camera>().Result;
}

// Prefer: use OnAppearing, a command, or async initialization
...
```csharp
// Avoid: handle only the granted case
if (status == PermissionStatus.Granted) { /* proceed */ }

// Prefer: handle the relevant statuses explicitly
switch (status)
{
...
### iOS: one-shot permission dialog
...
### Android: `ShouldShowRationale` timing
...
### Android API 33+: `StorageRead` and `StorageWrite` no longer help
...
```csharp
// Avoid: rely on StorageRead on API 33+
await Permissions.RequestAsync<Permissions.StorageRead>();

// Prefer: request the specific media permission
...
### Windows: most permissions return `Granted`
